...
def creat_table(data, parent_title='頂級菜單'):
    html=''
    for row in data:
        splice='├'
        id=row['id']
        ...
        tr_td="""<tr class='text-c'>
            <td><input type="checkbox" value="" name="smallBox"
            id="smallBox"></td>
            <td>{id}</td>
            <td class="text-1">{comment}<u style="cursor:pointer"
            class="text-primary" onClick="comment_edit('查看','{id}','')"
            title="查看"></u></td>
            <td>{title}</td>
            <td>{user_name}</td>
            <td>{add_time}</td>
            ...
            <a style="text-decoration:none" onClick="{status3}
            (this,'{id}')" href="javascript:;" title="{status1}">
            <i class="Hui-iconfont">{status2};</i></a>
            <a style="text-decoration:none" class="ml-5"
            onClick="comment_del(this,'{id}')" href="javascript:;" 
            title="刪除"><i class="Hui-iconfont">&#xe6e2;</i></a>
            </tr>"""
            
        if row['child']:
            html += tr_td.format(class_name='top_menu',
                                 title=title,
                                 id=id,
                                 comment=comment,
                                 status=status,
